...
proto._init = function _init(base, options) {
  ...
  this._connect(options, function(err, connection) {
    ...
    function onClose() {
      state.connection.removeListener('close', onClose);
      state.connection.removeListener('error', onError);
      self.emit('close');
    }
    ...
    state.connection.once('close', onClose);
    state.connection.once('error', onError);
